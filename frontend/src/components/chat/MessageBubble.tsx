...
export function MessageBubble({ message }: { message: Message }) {
  ...
  if (isSystem) {
    return (
      <div className="flex justify-center py-2">
        <span className="text-xs text-gray-600 italic px-3 py-1 bg-gray-900 rounded-full max-w-[90%] text-center">
          {message.content}
        </span>
      </div>
    );
  ...
  return (
    <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} group`}>
      {/* Role label */}
      <span className="text-[10px] px-1 mb-0.5 font-mono text-gray-600">
        {message.role}
      </span>
      ...
      >
        ...
        {/* Inspect button — always visible on touch, hover on desktop */}
        {hasDebug && isAssistant && (
          <button
            ...
            className="mt-1.5 flex items-center gap-1 px-2 py-1 rounded-md
                       ...
                       hover:text-purple-400 hover:border-purple-600
