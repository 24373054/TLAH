...
function DebugPanelInner({ turnId, onClose }: { turnId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (cancelled) return;
      if ('_error' in req) setError((req as { _error: string })._error);
      else setRawRequest(req as RawRequestData);
      if ('_error' in resp) { if (!error) setError((resp as { _error: string })._error); }
      else setRawResponse(resp as RawResponseData);
      setLoading(false);
    });
  ...
  const handleCopy = () => {
    ...
    if (data) { navigator.clipboard.writeText(JSON.stringify(data, null, 2)); setCopied(true); setTimeout(() => setCopied(false), 2000); }
  };
  ...
  return (
    <div className="fixed inset-0 z-40 md:flex md:flex-row">
      ...
      <div className="absolute inset-x-0 bottom-0 max-h-[85vh]
                      ...
                      bg-white dark:bg-gray-950 border-t md:border-l md:border-t-0
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-800 shrink-0">
          <div className="flex items-center gap-3">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-200">Raw API Inspector</h3>
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-800 shrink-0">
          <TabBtn active={tab === 'request'} onClick={() => setTab('request')} label="Raw Request"
          ...
          <TabBtn active={tab === 'response'} onClick={() => setTab('response')} label="Raw Response"
            icon={<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l9-9 9 9M5 10v10a1 1 0 001 1h3a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1h3a1 1 0 001-1V10" /></svg>} />
        </div>

        {/* Meta bar */}
        {activeData && (
          <div className="px-3 sm:px-4 py-2 bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200/50 dark:border-gray-800/50
                          flex items-center gap-2 sm:gap-4 text-[10px] text-gray-500 dark:text-gray-500 font-mono shrink-0 overflow-x-auto">
            {tab === 'request' && rawRequest && (
              <>
                <span>Provider: <span className="text-gray-700 dark:text-gray-400">{rawRequest.provider}</span></span>
                <span>Endpoint: <span className="text-gray-700 dark:text-gray-400 truncate max-w-[200px]">{rawRequest.endpoint_url}</span></span>
              </>
            )}
            {tab === 'response' && rawResponse && (
              <>
                <span>Status: <span className={rawResponse.http_status_code < 300 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}>{rawResponse.http_status_code}</span></span>
                <span>Latency: <span className="text-gray-700 dark:text-gray-400">{rawResponse.latency_ms}ms</span></span>
                {rawResponse.token_usage_json && (
                  <span>Tokens: <span className="text-gray-700 dark:text-gray-400">{Object.entries(rawResponse.token_usage_json).map(([k, v]) => `${k}=${v}`).join(', ')}</span></span>
                )}
              </>
            )}
          </div>
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto overscroll-contain p-4 bg-white dark:bg-gray-950">
          {loading ? (
            <div className="flex items-center justify-center h-full"><div className="flex items-center gap-3 text-gray-400 dark:text-gray-500"><Spinner /><span className="text-sm">Loading raw data...</span></div></div>
          ) : error ? (
            <div className="flex items-center justify-center h-full"><div className="text-center"><p className="text-red-600 dark:text-red-400 text-sm mb-2">Failed to load data</p><p className="text-gray-400 dark:text-gray-600 text-xs">{error}</p></div></div>
          ) : activeData ? (
            <div className="bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-4 overflow-x-auto">
              <JsonViewer data={tab === 'request' ? rawRequest!.request_json : rawResponse!.response_json} />
            </div>
          ) : null}
        </div>

        {/* Footer */}
        <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-800 flex items-center justify-between shrink-0">
          <p className="hidden sm:block text-[10px] text-gray-400 dark:text-gray-600">
            {tab === 'request'
          ...
          <button onClick={handleCopy} disabled={!activeData}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium
                             bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400
                             hover:text-gray-700 dark:hover:text-gray-200 border border-gray-300 dark:border-gray-700
                             hover:border-gray-400 dark:hover:border-gray-600 transition-colors disabled:opacity-50 shrink-0">
            {copied ? (
  ...
  return (
    <button onClick={onClick}
            className={`flex items-center gap-2 px-4 py-2.5 text-xs font-medium border-b-2 transition-colors duration-150
              ${active
                ? 'text-purple-600 dark:text-purple-400 border-purple-500 bg-purple-50 dark:bg-purple-500/5'
                : 'text-gray-500 dark:text-gray-500 border-transparent hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-900/50'
              }`}>
      {icon}{label}
    </button>
  );
